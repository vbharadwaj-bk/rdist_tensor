...
def test_tensor_evaluation():
    ...
    for i in range(dim):
        world_comm.Gatherv(ground_truth.tensor_idxs[i], [idx_buffers[i], counts, disps, MPI.UINT64_T])

    values = np.zeros(ground_truth.nnz, dtype=np.double) 
    world_comm.Gatherv(ground_truth.values, [values, counts, disps, MPI.DOUBLE])
